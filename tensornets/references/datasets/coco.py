...
from .ds_utils import xyxy_to_xywh, unique_boxes, filter_small_boxes, validate_boxes
...
class coco(imdb):
    def __init__(self, image_set, year, devkit_path=None, max_rows=None):
        ...
        self._data_path = osp.join(cfg.DATA_DIR, 'coco') if devkit_path is None \
                          else devkit_path
        ...
        self._image_index = self._load_image_set_index(max_rows)
    ...
    def _load_image_set_index(self, max_rows):
        """
        Load image ids.
        """
        image_ids = self._COCO.getImgIds()
        if max_rows is not None:
            return image_ids[:max_rows]
        else:
            return image_ids
